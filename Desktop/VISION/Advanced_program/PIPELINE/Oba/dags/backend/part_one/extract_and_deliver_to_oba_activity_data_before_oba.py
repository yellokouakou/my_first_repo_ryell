...
def extract_activity_data(spark, td_driver, td_url, td_user, td_password, beg_date, end_date, output_path):
    ...
    td_sql = "SELECT * FROM IGANB_DWH.OBA_ACTIVITY_DATA_SUBS WHERE CAST(DATEDATA AS DATE FORMAT 'yyyyMMdd') BETWEEN CAST('" + beg_date + "'  AS date FORMAT 'yyyyMMdd') AND CAST('" + end_date + "'  AS date FORMAT 'yyyyMMdd')"
    td_process.load(td_sql)
    td_process.to_table("td_oba_activity")

   # Normalizing the data
    td_df_oba_norm = spark.sql("""select concat("22507", right(user_id,8)) as mmid, datedata, balombeg, balomend, nbcashin, volcashin, nbcashout, volcashout, nbcharge, nbp2pin, volp2pin, nbp2pout, volp2pout, nbmerpay, volmerpay, nbbillpay, volbillpay, nbotherin, volotherin, nbotherout, volotherout, nbp2pphonein, nbp2pphoneout, nbtotalcharge, voltotalcharge, nbcall, timecall, nbsms, nbcontactcallin, nbcontactcallout, nbcontactsms, geoloccomm, geolocnight, preferedagent, preferedagentqual, mmdate, devicetype from td_oba_activity""")
    td_df_oba_norm.createOrReplaceTempView("td_df_oba_activity_norm")
    
    # Filtering out oba clients data
    td_df = spark.sql("""select mmid, datedata, balombeg, balomend, nbcashin, volcashin, nbcashout, volcashout, nbcharge, nbp2pin, volp2pin, nbp2pout, volp2pout, nbmerpay, volmerpay, nbbillpay, volbillpay, nbotherin, volotherin, nbotherout, volotherout, nbp2pphonein, nbp2pphoneout, nbtotalcharge, voltotalcharge, nbcall, timecall, nbsms, nbcontactcallin, nbcontactcallout, nbcontactsms, geoloccomm, geolocnight, preferedagent, preferedagentqual, mmdate, devicetype from td_df_oba_activity_norm inner join oba_clients on td_df_oba_activity_norm.mmid = oba_clients.Phone""")
...
def main():
    ...
    source_client_file = "/home/davidtia/output/AllCustomers_OBA_CI_new_last.csv"
    ...
    dates = generate_dates_from_weeks(4, args.edate)
    prefix = "activity_data_transactions_newclients_of_" + args.edate + "_" 
