...
def extract_marketing_data(spark, oracle_driver, oracle_url, oracle_user, oracle_pwd, a_date, output_path):
    oracle_process = ExtractorSQL(spark, True, oracle_driver, oracle_url, oracle_user, oracle_pwd)
    marketing_sql = "select msisdn, zone_commerciale, nom_region as region, nom_depart as departement, unite_administrative as commune, geolocalite, quartier, date_id from csp_oci.vbm_localisation_dly@old_to_csp where date_id ='" + a_date + "'"
    oracle_process.load(marketing_sql)
    oracle_process.to_table("oracle_marketing")

    # Normalizing the data
    oracle_df_oba_norm = spark.sql("""select concat("22507", right(msisdn,8)) as msisdn, zone_commerciale, region, departement, commune, geolocalite, quartier, date_id from oracle_marketing""")
    oracle_df_oba_norm.createOrReplaceTempView("oracle_marketing_norm")

    # Filtering out oba clients data
    oracle_df = spark.sql("""select msisdn, zone_commerciale, region, departement, commune, geolocalite, quartier, date_id from oracle_marketing_norm inner join oba_clients on oracle_marketing_norm.msisdn = oba_clients.Phone""")
    # oracle_df.show()
...
def extract_and_copyfile(args, spark, oracle_driver, oracle_url, oracle_user, oracle_password, a_date, prefix):
    ...
    extract_marketing_data(spark, oracle_driver, oracle_url, oracle_user, oracle_password, a_date, filename)
...
def main():
    ...
    if str(args.clients).find("new") != -1: # contains new clients
        ...
        prefix = "OBA_DATA_GEOMARKETING_NEWCLIENTS_OF_" + args.date + "_"
        for date in dates:
