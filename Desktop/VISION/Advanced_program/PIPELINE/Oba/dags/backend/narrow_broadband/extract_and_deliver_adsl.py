...
def extract_adsl_data(spark, oracle_driver, oracle_url, oracle_user, oracle_pwd, a_date, output_path):
    oracle_process = ExtractorSQL(spark, True, oracle_driver, oracle_url, oracle_user, oracle_pwd)
    adsl_sql = "select plan_tarifaire, debit, categorie, category_client as categorie_client, etat, contact, second_contact, ville, numero_adsl as numero from dwhdev.bub_fixe_parc_ods_adsl where date_jour='" + a_date + "'"
    oracle_process.load(adsl_sql)
    oracle_process.to_table("oracle_adsl")
    ...
    oracle_df_oba_norm = spark.sql("""select plan_tarifaire, debit, categorie, categorie_client, etat, case when rlike(contact,'^(225)[0456789]{1}[0-3]{1}[0-9]*$') then '22501'||right(contact,8)
        ...
        end second_msisdn, ville, numero from oracle_adsl""")
    oracle_df_oba_norm.createOrReplaceTempView("oracle_adsl_norm")

    # exploding the data with msisdn and second_msisdn
    oracle_df_oba_exploded = spark.sql("""(select plan_tarifaire, debit, categorie, categorie_client, etat, msisdn, ville, numero from oracle_adsl_norm) union all (select plan_tarifaire, debit, categorie, categorie_client, etat, second_msisdn as msisdn, ville, numero from oracle_adsl_norm)""")
    oracle_df_oba_norm.createOrReplaceTempView("oracle_adsl_norm")

    # Filtering out oba clients data
    oracle_df = spark.sql("""select msisdn, plan_tarifaire, debit, categorie, categorie_client, etat, ville, numero from oracle_adsl_norm inner join oba_clients on oracle_adsl_norm.msisdn = oba_clients.Phone""")
...
def main():
    ...
    user = "davidtia"
    password = "********"
    destination_folder = "/home/davidtia/incoming/recurrence/adsl/" # telco_data for historic data
    ...
    filename = "OBA_DATA_ADSL_" + a_date + "_RESULTS.csv"
    extract_adsl_data(spark, oracle_driver, oracle_url, oracle_user, oracle_password, a_date, filename)
