...
def extract_fdd_data(spark, oracle_driver, oracle_url, oracle_user, oracle_pwd, a_date, output_path):
    oracle_process = ExtractorSQL(spark, True, oracle_driver, oracle_url, oracle_user, oracle_pwd)
    fdd_sql = "select plan_tarifaire, 'FDD' as debit, categorie_offre as categorie, categorie_client, second_contact, ville, nd as numero from dwhdev.bub_fixe_parc_ods_fdd_v2 where date_jour='" + a_date + "'"
    oracle_process.load(fdd_sql)
    oracle_process.to_table("oracle_fdd")

    # Normalizing the data
    oracle_df_oba_norm = spark.sql("""select plan_tarifaire, debit, categorie, categorie_client, case when rlike(second_contact,'^(225)[0456789]{1}[0-3]{1}[0-9]*$') then '22501'||right(second_contact,8)
            ...
            when rlike(second_contact,'^(225)\d{10}$') then second_contact
        end msisdn, ville, numero from oracle_fdd""")
    oracle_df_oba_norm.createOrReplaceTempView("oracle_fdd_norm")

    # Filtering out oba clients data
    oracle_df = spark.sql("""select msisdn, plan_tarifaire, debit, categorie, categorie_client, ville, numero from oracle_fdd_norm inner join oba_clients on oracle_fdd_norm.msisdn = oba_clients.Phone""")
...
def main():
    ...
    user = "davidtia"
    password = "********"
    destination_folder = "/home/davidtia/incoming/recurrence/fdd/"
    ...
    filename = "OBA_DATA_FDD_" + a_date + "_RESULTS.csv"
    extract_fdd_data(spark, oracle_driver, oracle_url, oracle_user, oracle_password, a_date, filename)
