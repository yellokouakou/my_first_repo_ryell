...
def extract_tdd_data(spark, oracle_driver, oracle_url, oracle_user, oracle_pwd, a_date, output_path):
    oracle_process = ExtractorSQL(spark, True, oracle_driver, oracle_url, oracle_user, oracle_pwd)
    tdd_sql = "select plan_tarifaire, 'TDD' as debit, categorie_offre as categorie, categorie_client, second_contact, ville, nd as numero from dwhdev.bub_fixe_parc_ods_tdd_v3 where date_jour='" + a_date + "'"
    oracle_process.load(tdd_sql)
    oracle_process.to_table("oracle_tdd")
    ...
    oracle_df_oba_norm = spark.sql("""select plan_tarifaire, debit, categorie, categorie_client, case when rlike(second_contact,'^(225)[0456789]{1}[0-3]{1}[0-9]*$') then '22501'||right(second_contact,8)
        ...
        end msisdn, ville, numero from oracle_tdd""")
    oracle_df_oba_norm.createOrReplaceTempView("oracle_tdd_norm")

    # Filtering out oba clients data
    oracle_df = spark.sql("""select msisdn, plan_tarifaire, debit, categorie, categorie_client, ville, numero from oracle_tdd_norm inner join oba_clients on oracle_tdd_norm.msisdn = oba_clients.Phone""")
...
def main():
    ...
    user = "davidtia"
    password = "********"
    destination_folder = "/home/davidtia/incoming/recurrence/tdd/"
    ...
    filename = "OBA_DATA_TDD_" + a_date + "_RESULTS.csv"
    extract_tdd_data(spark, oracle_driver, oracle_url, oracle_user, oracle_password, a_date, filename)
